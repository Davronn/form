...
const getRow = ({
  ...
  return `
    <tr class="tabel gap-3">
      ...
      <td class = "text-end">
        <button class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#pupilModal" onclick="editPupil(${id})">O'zgartirish</button>
        <button class="btn btn-outline-danger" onclick="deletePupil(${id})">O'chirish</button>
      </td>
      </tr>
      `;
};
...
function deletePupil(id) {
  let isConfirm = confirm("O'chirilsinmi ?");
  if (isConfirm) {
